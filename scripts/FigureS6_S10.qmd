...
plot_it <- function(region, retrieve_osm = FALSE){
  ...
  if (retrieve_osm){
    ...
    # BUS STOPS
    bus_stop <- opq(bbox = bbox_larger) |>
      add_osm_feature(key = "highway",
                      value = "bus_stop") |>
      osmdata_sf()
    saveRDS(bus_stop, paste0("../data-raw/osm/bus_stop_",region,".rds"))
    ...
    # LANDUSE
    landuse <- opq(bbox = bbox_larger) |>
    add_osm_feature(key = "landuse",
                    value = c("residential", "commercial", "retail", "industrial", "mixed_use")) |>
      osmdata_sf()
    saveRDS(landuse, paste0("../data-raw/osm/landuse_",region,".rds"))
  ...
  # Read osm files
  bus_stop <- readRDS(paste0("../data-raw/osm/bus_stop_",region,".rds"))
  ...
  landuse <- readRDS(paste0("../data-raw/osm/landuse_",region,".rds"))
  place <- readRDS(paste0("../data-raw/osm/place_",region,".rds"))
  water <- readRDS(paste0("../data-raw/osm/water_",region,".rds"))

  # Get specific landuse info
  land <- landuse$osm_multipolygons
  ...
  mixed <- land |> 
    filter(landuse == 'mixed_use')
  
  greenspaces <- leisure$osm
  ...
  # City boundary
  city_boundary <- place$osm_points |> filter(place == "city")
  city_centroids <- city_boundary |> 
    group_by(name) |> 
    summarise(Center = st_centroid(geometry))
  
  # Now plotting
  p <- ggplot() + 
    geom_sf(data = leisure$osm_multipolygons, aes(fill = "Leisure"), color = "#d0ecb4") + 
    geom_sf(data = leisure$osm_polygons, aes(fill = "Leisure"), color = "#d0ecb4") + 
    geom_sf(data = water$osm_multipolygons, aes(fill = "Water"), color = "#abd4e0") + 
    geom_sf(data = water$osm_polygons, aes(fill = "Water"), color = "#abd4e0") + 
    ...
    #geom_sf(data = city_centroids, color = 'black') + 
    geom_sf(data = df, 
...
if (save_figures){
  ggsave('../figures/fig_s6.pdf', plot = a, width = 15, height = 10, units = 'cm')
  ggsave('../figures/fig_s7.pdf', plot = d, width = 15, height = 10, units = 'cm')
  ggsave('../figures/fig_s8.pdf', plot = e, width = 15, height = 10, units = 'cm')
  ggsave('../figures/fig_s9.pdf', plot = h, width = 15, height = 6, units = 'cm') # smaller bc weird dims
  ggsave('../figures/fig_s10.pdf', plot = s, width = 15, height = 10, units = 'cm')
}
```
