---
title: "04_Daily_analysis_no_int"
format: html
editor: visual
---
...
analysis <- lapply(regions, function(region){
  ...
  eqn_quasi <- TrafficCount ~ TrafficType + Station + # traffic info
    ...
    IsOAD:TrafficType + IsOAD:Holiday + 
    TrafficType:AQI_Cat 
  ...
  param_list <- c("IsOAD1", 
    ...
    # Save results
    filePath <- paste0("../results/04_Daily_no_int/",region,"_param_ci.csv")
    write.csv(conf, filePath)
    
    filePath <- paste0("../results/04_Daily_no_int/",region,"_model_summary.csv")
